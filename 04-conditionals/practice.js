...
var getTagName = function (str) {
    ...
    var tag2=str.slice(1,tag1.length);
    if (tag1[0]=="/")
        {
            if(tag==tag2)
            {
...
var magic8Ball = function (str) {
    var arr=["It is certain","It is decidedly so","Without a doubt","Yes definitely","You may rely on it","As I see it,yes","Most likely",'Outlook good',"Yes","Signs point to yes","Reply hazy try again","Ask again later","Better not tell you now","Cannot predict now","Concentrate and ask again","Don't count on it","My reply is no","My sources say no",'Outlook not so good',"Very doubtful"]
...
var interjectAt = function (msg,pos,str) {
    if ((typeof(msg)=="string")&&(typeof(pos)=="number")&&(typeof(str)=="string"))
    {
        if (str.length>pos)
            {
                var beginning = str.slice(0,pos);
                var end = str.slice(pos,str.length);
                var strWithInterjection = beginning + "-"+msg+"-" + end; 
        ...
        {
         return " expected first arg to be a string, second arg to be a number and third arg to be a string";   
        }
...
var randomInterject = function (str,msg) {
     var pos= Math.ceil(Math.random()*((str.length)-1));
      var beginning = str.slice(0,pos);
                var end = str.slice(pos,str.length);
                var strWithInterjection = beginning + "-"+msg+"-" + end; 
                return strWithInterjection;
